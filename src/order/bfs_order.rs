...
pub fn compute_tree<'a>(
    ...
    Vec<&'a Point>,
    HashSet<&'a Point>,
    MultiMap<&'a Point, &'a dyn Constraint>,
) {
...
pub fn compute_forest(index: &PointIndex) -> Vec<Vec<(Point, Vec<&dyn Constraint>)>> {
        ...
        .unique()
    {
        ...
        if forest
            .iter()
            .any(|(_, p, _)| p.contains(root) && p.contains(orbiter))
        {
            continue;
        }
        ...
        forest = forest
            ...
            .filter(|(_, p, _)| !points.is_superset(p))
            .collect();
        // Add this new tree.
    ...
    forest
        ...
        .map(|(t, _, mut s)| {
            t.into_iter()
                .map(|p| (p.clone(), s.remove(p).unwrap_or_else(|| Vec::new())))
